...
abstract class CombatantTest extends PHPUnit_Framework_TestCase {
    public function setUp() {
        ...
        // Create boolean generators for always false and always true
        $booleanFalseGenerator = new BooleanGenerator(0);
        $booleanTrueGenerator = new BooleanGenerator(1);
        
        // Create lowest and highest possible combatants, and a random one
        $this->weakCombatant   = $this->createCombatant($randomZero, $booleanFalseGenerator);
        $this->strongCombatant = $this->createCombatant($randomOne, $booleanTrueGenerator);
    ...
    public function strengthIsBetweenParameters() {
        $weakAttack   = $this->weakCombatant->getAttackStrength();
        $strongAttack = $this->strongCombatant->getAttackStrength();
        $randomAttack = $this->randomCombatant->getAttackStrength();
    ...
    /**
     * @test
     * @covers Combatant::receiveRetaliation
     */
    public function canReceiveNonFatalRetaliation() {
        ...
        $healthBefore = $this->weakCombatant->getHealth();
        $retaliation = $this->weakCombatant->receiveRetaliation($retaliation);
